...
function Login({setAuth,setUserName,setencrypt}) {
  ...
  const [inputData, setinputData] = useState({ email: "", password: "",login:false ,key:""});
...
if(inputData["key"]!=''){
  ...
  return (
    <Grid container spacing={3} style={{ margin: "auto" }}>
      ...
      <Grid item xs={10} style={{ margin: "auto" }}>
        <Button variant="contained" fullWidth onClick={e=>loginEventhandler()}>
         {inputData["login"]==true?<><CircularProgress /></>:"Go"} 
        </Button>
      </Grid>
    </Grid>
  );
